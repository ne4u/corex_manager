...
def get_captcha_challenge(
    ...
    # server-side with a short TTL covering only the redirect -> page load.
    # This keeps internal rule details (rule_id, rule_type, rule_name,
    # request_id) and the redirect destination out of the URL entirely,
    # preventing open redirect attacks.
    #
    # The challenge page is only served for a real issued challenge: a
    # missing, expired, already-solved, or fabricated token gets an error
    # page instead of a solvable widget. Otherwise anyone could request this
    # URL directly, solve the rendered challenge, and mint a _cv bypass
    # cookie without any rule ever matching their traffic.
    context = cache_get(f"cap:cid:{cid}") if cid else None
    if not (context and isinstance(context, dict)):
        return HTMLResponse(
            render_error_page(
                "Challenge Expired",
                "This security check link is invalid or has expired. Return to the page you were visiting to start a new one.",
                "",
                400,
            ),
            status_code=400,
        )
    rule_id = int(context.get("i", 0) or 0)
    ...
    # The verify endpoint resolves everything it needs (rule attribution,
    # redirect target) from the server-side context — the only client input
    # it needs is the cid token itself.
    hidden_fields = {"cid": cid}
    ...
    # Keep the token alive until verify: re-store it with the solve-window
    # TTL (the Lua-side TTL only covers redirect -> page load). The "l" flag
    # marks that the issued event was already logged so a refresh or
    # re-render of the same challenge page doesn't double-count. The token
    # is finally consumed (deleted) by the verify endpoint on a successful
    # solve.
    first_render = not context.get("l")
    if first_render:
        context["l"] = 1
        _log_challenge_event(db, rule_id, rule_type, rule_name, "issued", request_id=request_id or None)
    cache_set(f"cap:cid:{cid}", context, ttl=cfg.CAPTCHA_CHALLENGE_TTL_SECONDS)
...
async def verify_captcha(
    ...
    # is issued by HAProxy's captcha_store_ctx Lua action when a challenge
    # rule matches, and its TTL is extended when the challenge page renders.
    # Without a live context there is no real challenge — refuse rather than
    # mint a _cv bypass cookie for an arbitrary POST. Rule attribution and
    # the post-solve redirect target come from the stored context, so
    # client-supplied form fields can no longer forge stats or redirects.
    context = cache_get(f"cap:cid:{cid}") if cid else None
    if not (context and isinstance(context, dict)):
        return HTMLResponse(
            render_error_page(
                "Challenge Expired",
                "This security check has expired or was already completed. Return to the page you were visiting to start a new one.",
                "",
                400,
            ),
            status_code=400,
    ...
    rule_name = str(context.get("n", ""))
    request_id = str(context.get("r", ""))
    redirect = _same_origin_redirect(str(context.get("u", "/")) or "/", request)
    ...
    # Consume the challenge token so each issued challenge can mint exactly
    # one _cv cookie. Failed solve attempts leave it in place so the user can
    # retry within the solve window.
...
def _same_origin_redirect(url: str, request: Request) -> str:
    """Return ``url`` if its host matches the live request's, else ``"/"``.

    The stored redirect target was built by HAProxy from the original
    request's Host header, which is client-controlled on listeners without
    host restrictions. Without this check an attacker could craft a challenge
    link whose stored target points at an external origin and hand it to a
    victim (open redirect). The verify request always arrives on the same
    listener/host as the challenge, so the live request's hostname is the
    authoritative same-origin reference. Relative URLs and explicit
    http(s) URLs on the same host are allowed; everything else (other hosts,
    non-HTTP schemes like ``javascript:``, protocol-relative ``//host``,
    ...
    if parsed.netloc:
        ...
        if not req_host or (parsed.hostname or "") != req_host:
            return "/"
    return url
...
def _log_challenge_event(db: Session, rule_id: int, rule_type: str, rule_name: str, event_type: str, request_id: Optional[str] = None) -> None:
    ...
    are only ever issued by rules, and HAProxy's captcha_store_ctx action is
    always emitted with a real rule id — a rule_id of 0 means the request
    carried no server-side challenge context (e.g. a direct POST to the verify
    endpoint or a form rendered from an expired/invalid cid token). Recording
    those would create stats rows that belong to no rule.
    """
    if rule_id <= 0:
        return
    try:
