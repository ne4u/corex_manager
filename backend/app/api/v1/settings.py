...
from ...models.models import NetworkList, Setting
...
def update_setting_endpoint(
    ...
    if key == "password_rotation_months":
        ...
        if val < 0 or val > 24:
            raise HTTPException(
                status_code=400, detail="password_rotation_months must be between 0 and 24 (0 = disabled)"
            )
    ...
    if key == "req_fp_enabled":
        ...
        if not enabling:
            ...
            api_armor_on = get_setting(db, "api_armor_enabled", str(_settings.API_ARMOR_ENABLED)).lower() in (
                "true",
                "1",
                "yes",
            )
