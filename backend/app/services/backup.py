...
# JSON columns that embed secret values which must be stripped when
# include_secrets=False. For vector_sinks.options, any value prefixed with
# "enc:" is Fernet-encrypted secret material — remove it so a non-secret
# backup can't carry decryptable ciphertext. The sink row itself is kept so
# the pipeline topology is preserved (secrets must be re-entered on restore).
            ...
            for row in db.execute(table.select().execution_options(yield_per=500)).mappings():
                ...
                if not include_secrets and name == "settings":
                    if row_dict.get("key") in SECRET_SETTING_KEYS:
                        continue
                if not include_secrets:
                    _redact_json_secrets(name, row_dict)
                if not first_row:
                    f.write(b",")
                first_row = False
...
def _collect_files(db: Session, include_secrets: bool, include_metrics: bool) -> Tuple[List[Tuple[str, str]], List[Tuple[str, str]]]:
    ...
    # Vector pipeline config — contains inlined plaintext sink credentials, so
    # it is only archived when secrets are included. On restore it is also
    # regenerated by write_config(), so the archived copy is a convenience.
    if include_secrets:
        files.append((settings.VECTOR_CONFIG_PATH, "config/vector.toml"))
        files.append((f"{settings.VECTOR_CONFIG_PATH}.applied", "config/vector.toml.applied"))
...
def restore_export(
    ...
    config_files = [
        ...
        ("config/vector.toml", settings.VECTOR_CONFIG_PATH),
        ("config/vector.toml.applied", f"{settings.VECTOR_CONFIG_PATH}.applied"),
    ]
