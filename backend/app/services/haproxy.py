...
def _maybe_peers(db: Optional[Session] = None) -> str:
    ...
    peers section is present (≥2 HAProxy instances). Returns empty string
...
def _generate_combined_lua_loader(include_geoip: bool, include_compress: bool, include_resp_transform: bool = False, include_img_2_webp: bool = False, include_api_armor: bool = False, include_req_fp: bool = False) -> str:
    ...
    break the others. The cross-.so symbol conflict is prevented at the linker
    level via ``-Wl,-Bsymbolic`` on each cdylib crate (emitted by a per-module
    ``build.rs``), which binds each .so's internal references to its own
    definitions so RTLD_GLOBAL dlopen in later threads can't redirect them to
    the first-loaded .so's symbols.
    """
    parts = [
        "-- Auto-generated by haproxy.py. Do not edit; regenerated on each config write.\n"
    ...
    if include_req_fp:
        parts.append(
            "-- Request fingerprint (lua.req_fp_capture action)\n"
...
def generate_global_section(
    ...
    #
    # The cross-.so symbol conflict is prevented at the linker level: each cdylib
    # crate's build.rs emits -Wl,-Bsymbolic, which binds the .so's internal
    # references to its own definitions at link time. This lets lua-load-per-
    # thread give each thread its own Lua state with per-thread dlopen of the
    # .so files without cross-thread symbol clobbering. HAProxy 3.4+ requires
    # that every referenced Lua function/filter be registered in ALL per-thread
    # states or none; without -Bsymbolic, dlopen RTLD_GLOBAL corruption causes
    # silent registration failures in later threads, triggering:
    #   "Lua function 'X' is not referenced in all thread."
    ...
    #
    # ja4.lua, acme.lua and risk_score.lua are stateless, so they are loaded
    # with lua-load-per-thread: each worker thread gets its own Lua state and
    # runs the actions/fetches without taking the global Lua lock that a plain
    # `lua-load` (single shared state) imposes on every thread.
    if ja4_enabled:
        lines.append("    tune.ssl.capture-buffer-size 336")
        lines.append("    lua-load-per-thread /etc/haproxy/ja4.lua")
    ...
    # action and native http-response set-var-fmt directives are emitted only
    # when req_fp_enabled.
    ...
    lines.append("    lua-load-per-thread /etc/haproxy/acme.lua")
    ...
    # per-ruleset vars to "0"). The rules data file is read once per Lua state
    # at load time; every apply reloads HAProxy, which re-creates the states.
    if req_fp_enabled:
        lines.append("    lua-load-per-thread /etc/haproxy/risk_score.lua")
...
def generate_frontend(
    ...
    if effective_mode == "http":
        ...
        if needs_stick_table:
            ...
            lines.append(f"    stick-table type ip size 1m expire {expire}s store {','.join(stores)}{_maybe_peers(db)}")
        ...
        # http-req. Native http-response set-var-fmt directives append the
        # final 2 fields (status, body_bytes). Gated by req_fp_enabled AND
        ...
        if req_fp_enabled and _req_fp_module_available() and not force_https_redirect:
            ...
            # appends status and body_bytes to produce the full 17-field
            # fingerprint (txn.req_fp) in the response phase. Both actions run
            # in the Rust module, keeping the fingerprint logic in one place.
            #
...
def generate_backend(
    ...
    if backend.stick_table:
        ...
        lines.append(f"    stick-table type {st_type} size {st_size} expire {st_expire}{_maybe_peers(db)}")
        if st_type == "ip":
...
def generate_coraza_spoe_config(db: Session = None, coraza_backend_name: str = "coraza-spoa") -> str:
    ...
    export_ids = "true"
    # SPOE frame size: HAProxy only accepts [256, tune.bufsize - 4], and
    # Coraza SPOA's internal maximum is 65535. Without an explicit value
    # HAProxy defaults to tune.bufsize - 4, which exceeds Coraza's limit when
    # tune.bufsize is raised ("maxFrameSize bigger than maximum allowed size"
    # errors), so pin it to the smaller of the two bounds.
    max_frame_size = max(256, min(settings.CORAZA_SPOE_MAX_FRAME_SIZE, _effective_tune_bufsize_from_db(db) - 4))
...
def generate_config(
    ...
    config += _generate_beacon_trust_tables(db)
...
def generate_mcp_upstreams(db: Session) -> str:
    ...
    for server in multi_servers:
        ...
        be_lines = [
            ...
            f"    stick-table type string len 128 size 10k expire 1h{_maybe_peers(db)}",
...
def _generate_waf_rate_backends(db: Session) -> str:
    ...
    for listener in db.query(Listener).all():
        ...
        sections.append(
            ...
            f"    stick-table type string len 256 size 1m expire {expire}s store gpc1_rate({window}s){_maybe_peers(db)}\n"
        )
    return "".join(sections)
...
def _generate_rl_rate_backends(db: Session) -> str:
    ...
    for listener in db.query(Listener).all():
        ...
        sections.append(
            f"\nbackend {backend_name}\n"
            f"    stick-table type string len 256 size 1m expire {expire}s store {','.join(stores)}{_maybe_peers(db)}\n"
        )
    return "".join(sections)
...
def _generate_block_tables(db: Session) -> str:
    ...
    for listener in db.query(Listener).all():
        ...
        if ip_table not in seen_names:
            ...
            sections.append(
                f"\nbackend {ip_table}\n"
                f"    stick-table type ip size 1m expire {max_duration}s store gpc0{_maybe_peers(db)}\n"
            )
        if needs_str_table:
            ...
            if str_table not in seen_names:
                ...
                sections.append(
                    f"\nbackend {str_table}\n"
                    f"    stick-table type string len 256 size 1m expire {max_duration}s store gpc0{_maybe_peers(db)}\n"
                )
    return "".join(sections)
...
def _generate_resp_code_tables(db: Session) -> str:
    ...
    for listener in db.query(Listener).all():
        ...
        sections.append(
            f"\nbackend {table_name}\n"
            f"    stick-table type ip size 1m expire {expire}s store gpc0_rate({max_window}s){_maybe_peers(db)}\n"
        )
    return "".join(sections)
...
def _generate_beacon_trust_tables(db: Optional[Session] = None) -> str:
    ...
    - ``cxid_table``: string-keyed, stores cxids (UUIDs) inserted into the
      ...
      BEACON_CXID_TTL_SECONDS (default 120s). Sized at 1m slots to
      accommodate per-request cardinality (one entry per HTML response).
    ...
    cxid_ttl = getattr(settings, "BEACON_CXID_TTL_SECONDS", 120)
    return (
        f"\nbackend cxid_table\n"
        f"    stick-table type string len 64 size 1m expire {cxid_ttl}s store http_req_cnt{_maybe_peers(db)}\n"
        f"\nbackend beacon_trust_table\n"
        f"    stick-table type ip size 100k expire {trust_ttl}s store http_req_cnt{_maybe_peers(db)}\n"
    )
...
def write_config(
    ...
    print("[WRITE_CONFIG] step 10 — dataplane.push_config (all instances)", flush=True)
    logger.info("write_config: step 10 — dataplane.push_config (all instances)")
    if settings.DATAPLANE_API_ENABLED:
        # Push to all configured HAProxy instances (single instance when HA
        # is off — preserves the original single-push behavior; multi-instance
        # when HA is on — pushes to each instance's Data Plane API).
        push_results = ha_service.push_config_to_all_instances(db, config)
        for inst_name, result in push_results.items():
            status = result.get("status", "unknown")
            if status == "error":
                logger.warning("Config push to %s failed: %s", inst_name, result.get("message", ""))
            else:
                logger.info("Config push to %s: %s", inst_name, status)
    # Write keepalived config files when HA is on
