...
from ..services.settings import get_maxmind_license_key, get_setting
...
PROVIDER_FIELD_OVERRIDES: dict[str, dict[str, Any]] = {
    ...
    "waf_exception": {
        "skip": {"update_action", "update_target", "condition_variable", "condition_operator", "condition_value"},
    },
    "waf_rule": {
        "skip": {
            "rule_set_plugins",
    ...
    "cache_config": {
        "skip": {
            "name",
            "haproxy_max_object_size",
            "haproxy_max_secondary_entries",
            "haproxy_cache_condition",
            "haproxy_process_vary",
            "disk_cache_grace",
            "disk_cache_purge_enabled",
        },
        "rename": {
...
SKIP_FIELDS: set[str] = {
    ...
    "status_codes",
    "first_seen",
    "last_seen",  # api_profiles (excluded)
    "cert_path",
    "key_path",
    "chain_path",  # system paths — export PEM files instead
}
...
SKIP_SETTING_KEYS: set[str] = {
    ...
    "page_protect_monitoring_enabled",
    "page_protect_change_detection_enabled",
    "page_protect_change_detection_interval_hours",
    "page_protect_report_retention_days",
    "page_protect_report_path",
    "page_protect_baseline_start",
    "page_protect_baseline_end",
    "page_protect_baseline_note",
    "page_protect_beacon_injection_enabled",
    "page_protect_beacon_trust_enabled",
    "page_protect_beacon_path",
    "page_protect_beacon_script_path",
    "page_protect_beacon_content_types",
    "page_protect_beacon_path_patterns",
    "page_protect_beacon_backend_ids",
    ...
    "api_armor_enabled",
    "api_armor_max_body_bytes",
    "api_armor_module_enabled",
    # Vector log pipeline sources (exported as corex_vector_sources singleton)
    "vector_sources",
    "api_armor_schema_learning_enabled",
    "api_armor_profiling_learning_enabled",
    "api_armor_profile_retention_days",
    "api_armor_scope",
...
def _hcl_map(val: dict) -> str:
    if not val:
        return "{}"
    items = ", ".join(
        f"{_hcl_string(k, allow_heredoc=False)} = {_hcl_value(v, allow_heredoc=False)}" for k, v in val.items()
    )
    return f"{{{items}}}"
...
class Module:
    ...
    def add_output(self, name: str, value: str, description: str = ""):
        ...
        self.outputs.append(f'output "{name}" {{\n{desc}  value = {value}\n}}')
        ...
        for field in fields:
            ...
            # and locals always exist for every type.
                ...
                if not type_attempts:
                    ...
                    coalesce_args = ", ".join(
                        f"try(local.{type_key}_list_ids[each.value.{attr_name}], null)" for type_key, _ in type_attempts
                    )
...
class TerraformExporter:
    ...
    def _resolve_id_ref(
        self, attrs: list[tuple[str, str]], field_name: str, id_val, var_name: str, name_map: dict[int, str]
    ) -> list[tuple[str, str]]:
    ...
    def _resolve_id_list_ref(
        self, attrs: list[tuple[str, str]], field_name: str, id_list, var_name: str, name_map: dict[int, str]
    ) -> list[tuple[str, str]]:
    ...
    def _add_secret_map_var(
        self, mod: Module, var_name: str, description: str, name_map_key: str = None, var_type: str = None
    ):
    ...
    def _build_object_type(
        ...
        mapper = sa_inspect(model_cls)
        all_fk_fields = (
            set(same_module_fks.keys())
            | set(cross_module_fks.keys())
            | set(optional_cross_module_fks.keys())
            | set(polymorphic_fks.keys())
        )
    ...
    def _add_for_each_collection(
        ...
        ordered_fields = [
            f
            for f in all_fields_minus_name
            if f not in skip_fields and f not in provider_skip and f not in provider_raw_int_skip
        ]
    ...
    def _build_ssl_module(self) -> Module:
        ...
        if certs:
            ...
            cert_fields_filtered = [
                f for f in simple_fields if f not in cert_overrides["skip"] and f not in cert_overrides["raw_int_skip"]
            ]
            mod.add_for_each_resource(
                resource_type="certificate",
        ...
        ssl_labs_type = "map(object({\n    max_scans_per_host = optional(number)\n  }))"
        self.collection_types["ssl_labs_settings"] = ssl_labs_type
        mod.add_variable(
            "ssl_labs_settings", ssl_labs_type, "SSL Labs settings per certificate (singleton per cert)", "{}"
        )
        if ssl_labs_data:
    ...
    def _build_traffic_module(self) -> Module:
        mod = Module(
            "traffic", "Traffic management: rate limits, headers, redirects, rewrites, transforms, error pages"
        )
        ...
        if error_pages:
            ...
            last_block = mod.blocks[-1]
            mod.blocks[-1] = last_block.replace(
                "  name = try(each.value.name, each.key)", "  code = try(each.value.code, each.key)"
            )
    ...
    def _build_security_lists_module(self) -> Module:
        mod = Module(
            "security-lists",
            "Security lists: network, ASN, geo, JA4, pattern lists with inline entries and dynamic feeds",
        )
        ...
        for label, list_cls, type_key in list_types:
            ...
            mod.add_collection_variable(
                f"{type_key}_lists", f"Map of {label.lower()} list name to configuration", var_type=list_obj_type
            )
            mod.add_variable(
                f"{type_key}_list_entries", "any", f"Map of {label.lower()} list name to entries list", "{}"
            )
            ...
            for lst in lists:
                ...
                self.extra_files[f"environments/dev/files/security-lists/{filename}"] = json.dumps(
        ...
        for type_key, res_type in all_list_types:
            local_lines.append(f"  {type_key}_list_ids = merge(")
            local_lines.append(f"    {{ for k, v in corex_{res_type}.this : k => v.id }},")
            local_lines.append(f"    {{ for k, v in corex_{res_type}.feed_managed : k => v.id }}")
            local_lines.append("  )")
        local_lines.append("}")
        ...
        for type_key, _ in all_list_types:
            mod.add_local_output(
                f"{type_key}_list_ids", f"{type_key}_list_ids", f"Merged {type_key} list IDs (this + feed_managed)"
            )
    ...
    def _build_cache_module(self) -> Module:
        ...
        mod.add_collection_variable(
            "cache_configs", "Map of cache config name to configuration", var_type=cache_config_type
        )

        cache_configs = self._query_all(CacheConfig)
        if cache_configs:
            self._register_names("cache_configs", cache_configs, name_col="backend_id")
            ...
            cc_fields = [f for f in config_fields if f not in cc_provider_skip and f not in cc_provider_raw_int_skip]
    ...
    def _build_observability_module(self) -> Module:
        ...
        mod.add_variable("vector_sources", "any", "Vector log sources (corex/waf/mcp booleans)", "{}")
        ...
        mod.add_collection_variable("vector_sinks", "Map of vector sink name to configuration", var_type=sink_type)
        self._add_secret_map_var(
            mod,
            "vector_sink_secrets",
    ...
    def _build_page_protect_module(self) -> Module:
        ...
        if pp_data:
            ...
            for k in [
                ...
                "change_detection_interval_hours",
                "report_retention_days",
                "report_path",
                "beacon_injection_enabled",
                "beacon_trust_enabled",
                "beacon_paths",
                "beacon_content_types",
                "beacon_patterns",
                "backend_ids",
                "auto_prune_stale_days",
            ]:
    ...
    def _export_singleton_settings(self, mod: Module, db) -> None:
        ...
        global_opts = get_setting(db, "haproxy_global_options")
        global_opts_type = (
            "list(object({ target = optional(string) "
        ...
        mod.add_variable("haproxy_global_options", global_opts_type, "HAProxy global options (list of objects)", "[]")
        if global_opts:
            mod.add_section("HAProxy Global Options")
            mod.blocks.append('resource "corex_global_options" "this" {\n  options = var.haproxy_global_options\n}')
        ...
        if not (self.include_secrets or self.include_system_secrets):
            ...
            self._add_secret_map_var(
                mod, "captcha_secrets", "Captcha secret keys (cap_secret, recaptcha_secret, turnstile_secret)"
            )
            if captcha_data:
        ...
        if maxmind_key:
            if self.include_secrets or self.include_system_secrets:
                ...
                mod.blocks.append('resource "corex_maxmind_license_key" "this" {\n  value = var.maxmind_license_key\n}')
            else:
    ...
    def _build_mcp_gateway_module(self) -> Module:
        ...
        self._add_for_each_collection(
            ...
            name_fn=lambda ut: (
                f"member_{mcp_team_map.get(ut.team_id, 'unknown')}_{user_map.get(ut.user_id, 'unknown')}"
            ),
        ...
        if not (self.include_secrets or self.include_system_secrets):
            ...
            self._add_secret_map_var(
                mod, "mcp_servers_auth_secrets", "MCP server auth secrets", name_map_key="mcp_servers"
            )
            self._add_secret_map_var(
                mod, "mcp_servers_oauth_secrets", "MCP server OAuth client secrets", name_map_key="mcp_servers"
            )
        ...
        skill_versions = [sv for sv in self._query_all(McpSkillVersion) if sv.skill_id not in self_reg_skill_ids]
        if skill_versions:
            for sv in skill_versions:
                ...
                if row.get("frontmatter"):
                    ...
                    self.extra_files[f"environments/dev/files/mcp-gateway/{fname}"] = json.dumps(
                        row["frontmatter"], indent=2
                    )
...
"""
    ...
    def _build_tfvars_data(self) -> dict[str, Any]:
        ...
        self._add_tfvars_collection(
            tfvars, "cipher_suites", CipherSuite, value_transforms={"tls_options": lambda v: v.split() if v else []}
        )
        ...
        self._add_tfvars_collection(
            tfvars, "backends", Backend, same_module_fk_maps={"fcgi_app_id": "fcgi_apps"}, resource_type="backend"
        )
        self._add_tfvars_collection(
            tfvars, "servers", Server, same_module_fk_maps={"backend_id": "backends"}, resource_type="server"
        )
        self._add_tfvars_collection(
            tfvars,
            "listeners",
        ...
        # Traffic module
        self._add_tfvars_collection(
            tfvars, "error_pages", CustomErrorPage, name_fn=lambda ep: str(ep.code), skip_fields={"content"}
        )
        self._add_tfvars_collection(tfvars, "rate_limits", RateLimit, cross_module_fks={"listener_id": "listeners"})
        self._add_tfvars_collection(
            tfvars,
            "response_headers",
            ResponseHeader,
        ...
        # Page protect
        self._add_tfvars_collection(
            tfvars, "page_protect_policies", PageProtectPolicy, fk_list_fields={"backend_ids": "backends"}
        )
        self._add_tfvars_collection(
            tfvars,
            "page_protect_scripts",
        ...
        if api_key_entries:
            ...
            for ent in api_key_entries:
                ...
                if list_key:
                    entries_by_list.setdefault(list_key, []).append({"value": ent.value, "note": ent.note})
        ...
        self._add_tfvars_collection(tfvars, "api_schemas", ApiSchema, same_module_fk_maps={"spec_id": "openapi_specs"})

        # Risk scoring
        self._add_tfvars_collection(tfvars, "risk_rulesets", RiskRuleset)
        self._add_tfvars_collection(tfvars, "risk_rules", RiskRule, same_module_fk_maps={"ruleset_id": "risk_rulesets"})
        ...
        self._add_tfvars_collection(
            tfvars, "users", User, name_attr="username", secret_fields=user_secret_fields, resource_type="user"
        )
...
"""
    ...
    def _generate_imports_tf(self, modules: dict[str, Module]) -> str:
        ...
        for var_name, resource_type, mod_name, id_mode, model_cls in import_specs:
            ...
            # via ListBackends, making the import portable across instances.
                ...
                backend_name_by_id = {b.id: b.name for b in self._query_all(Backend)}
                rows = self._query_all(model_cls)
                for row in rows:
                    tf_key = name_map.get(row.id)
                    if not tf_key:
            ...
            # via ListMcpTeams and ListUsers, making the import portable.
                ...
                team_name_by_id = {t.id: t.name for t in self._query_all(Team)}
                user_username_by_id = {u.id: u.username for u in self._query_all(User)}
                rows = self._query_all(model_cls)
                for row in rows:
                    tf_key = name_map.get(row.id)
                    if not tf_key:
            ...
            # so the import ID must be the numeric cert ID from the DB.
                ...
                cert_id_by_slug = {_sanitize_name(c.name): c.id for c in self._query_all(Certificate)}
                for key in col_data:
    ...
    def _generate_outputs_tf(self, modules: dict[str, Module]) -> str:
        ...
        root_outputs = {
            ...
            "ssl": ["certificate_ids"],
            "management": ["user_ids"],
            "security-lists": ["network_list_ids", "asn_list_ids", "geo_list_ids", "ja4_list_ids", "pattern_list_ids"],
            "waf": ["waf_rule_ids"],
...
Auto-generated by coreX Manager Terraform Export on {datetime.now(UTC).strftime("%Y-%m-%d %H:%M:%S UTC")}.
