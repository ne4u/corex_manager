...
class CacheMetricSnapshot(Base):
    ...
    backend_id = Column(
        Integer, ForeignKey("backends.id", ondelete="CASCADE"), nullable=True, index=True
...
class SslLabsScan(Base):
    ...
    __table_args__ = (Index("ix_ssllabs_scans_cert_host_created", "certificate_id", "host", "created_at"),)
