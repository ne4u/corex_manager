...
def upgrade() -> None:
    ...
    result = bind.execute(sa.text("SELECT 1 FROM risk_rulesets WHERE id = 1")).fetchone()
    if result is None:
        op.execute(
    ...
    if "ruleset_id" not in risk_columns:
        ...
        with op.batch_alter_table("risk_rules", schema=None) as batch_op:
            batch_op.alter_column("ruleset_id", existing_type=sa.Integer(), nullable=False)
            batch_op.create_foreign_key(
                "fk_risk_rules_ruleset_id",
                "risk_rulesets",
                ["ruleset_id"],
                ["id"],
                ondelete="CASCADE",
