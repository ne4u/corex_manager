...
def test_graphql_depth(db):
    ...
    assert 'var(txn.gql_depth)' in cond
...
def test_graphql_complexity(db):
    ...
    assert 'var(txn.gql_complexity)' in cond
...
def test_graphql_operation(db):
    ...
    assert 'var(txn.gql_operation)' in cond
...
def test_var_bool_field_requires_match_method(db):
    ...
    Legacy risk/keep_alive bools store '0'/'1' strings, so use -m str 1.
    Rust API Armor bools are real HAProxy booleans and use -m bool.
    """
    # Bare bool_field
    ...
    assert '-m str 1' in cond
    assert '!{' in cond
    # = true comparison (Rust bool)
    cond = _tr('auth.valid = true', db)
    assert '-m bool' in cond
    # = false comparison (Rust bool)
...
def test_api_schema_valid_bool(db):
    ...
    cond = _tr('api.schema_valid = false', db)
    assert 'var(txn.api_schema_valid)' in cond
...
def test_api_schema_errors(db):
    ...
    assert 'var(txn.api_schema_errors)' in cond
...
def test_auth_valid_bool(db):
    ...
    cond = _tr('auth.valid = true', db)
    assert 'var(txn.auth_valid)' in cond
...
def test_auth_type(db):
    ...
    assert 'var(txn.auth_type)' in cond
