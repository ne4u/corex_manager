...
def _populate_full_config(db):
    ...
    db.add(ResponseHeader(name="hsts", listener_id=ln.id, header="Strict-Transport-Security", value="max-age=31536000"))
    ...
    # Security rules
    make_security_rule(
        db, name="block_bots", listener_ids=[ln.id], action="deny", expression="hdr(user-agent) -m sub bot"
    )
    ...
    db.add(
        VectorSink(
            ...
            type="aws_s3",
            source="corex",
            options={"bucket": "logs", "region": "us-east-1", "access_key_id": "AKIA...", "secret_access_key": "shh"},
        )
    )
    ...
    db.add(
        ApiSchema(name="schema1", method="GET", path="/api", schema='{"type":"object"}', source="manual", enabled=True)
    )
    ...
    # Management
    db.add(User(username="admin", email="admin@ex.com", role="admin", hashed_password="$2b$12$abc", is_admin=True))
    ...
    db.add(skill)
    db.flush()
    db.add(McpSkillVersion(skill_id=skill.id, version="1.0.0", body="# Helper", frontmatter={"v": "1.0.0"}))
...
class TestFKResolution:
    ...
    def test_cross_module_fk_in_module_main_tf(self, db):
        ...
        assert "fcgi_app_id" not in routing_tf, "fcgi_app_id should be skipped (not in provider backend schema)"
    ...
    def test_polymorphic_fk_uses_coalesce_not_key(self, db):
        ...
        assert ".key]" not in feed_block, "Polymorphic FK should not use .key (value is a string)"
    ...
    def test_polymorphic_fk_multiple_asn_feeds(self, db):
        ...
        for i, name in enumerate(asn_names, 1):
            db.add(
                DynamicFeed(
                    name=f"feed_{name.replace('asn_', '')}",
                    list_type="asn",
                    target_list_id=i,
...
class TestSecretFlags:
    def _populate_with_secrets(self, db):
        ...
        db.add(Certificate(name="wildcard", domain="*.example.com", dns_credentials="SECRET_DNS_CREDENTIALS"))
        team = Team(name="Eng", slug="eng")
        db.add(team)
        db.flush()
        srv = McpServer(
            name="tools", team_id=team.id, namespace="eng", transport_type="stdio", auth_secret_enc="SECRET_AUTH_TOKEN"
        )
        db.add(srv)
        db.flush()
        db.add(
            McpIdentity(
                name="bot", team_id=team.id, kind="pat", pat_hash="SECRET_PAT_HASH", idp_user_info={"sub": "secret"}
            )
        )
    ...
    def test_all_flags_off_no_secrets_leaked(self, db):
        ...
        zf = _export(
            db, include_secrets=False, include_certs=False, include_users_identities=False, include_system_secrets=False
        )
        tv = _tfvars(zf)
    ...
    def test_include_secrets_master_override(self, db):
        ...
        self._populate_with_secrets(db)
        zf = _export(
            db, include_secrets=True, include_certs=False, include_users_identities=False, include_system_secrets=False
        )
        tv = _tfvars(zf)
        for s in self.SECRET_VALUES:
    ...
    def test_include_certs_only(self, db):
        ...
        self._populate_with_secrets(db)
        zf = _export(
            db, include_secrets=False, include_certs=True, include_users_identities=False, include_system_secrets=False
        )
        tv = _tfvars(zf)
    ...
    def test_include_users_identities_only(self, db):
        ...
        self._populate_with_secrets(db)
        zf = _export(
            db, include_secrets=False, include_certs=False, include_users_identities=True, include_system_secrets=False
        )
        tv = _tfvars(zf)
    ...
    def test_include_system_secrets_only(self, db):
        ...
        self._populate_with_secrets(db)
        zf = _export(
            db, include_secrets=False, include_certs=False, include_users_identities=False, include_system_secrets=True
        )
        tv = _tfvars(zf)
        ...
        non_sys = ["SECRET_HASHED_PASSWORD", "SECRET_TOTP", "SECRET_PAT_HASH", "SECRET_DNS_CREDENTIALS"]
        for s in non_sys:
    ...
    def test_secret_variables_declared_in_variables_tf(self, db):
        ...
        for var_name in ["mcp_servers_auth_secrets", "certificates_dns_credentials"]:
...
class TestSecretPlaceholderFormat:
    ...
    ALL_SECRET_VALUES = [
        ...
        "SECRET_MAXMIND_KEY",
        "SECRET_CAP_SECRET",
        "SECRET_RECAPTCHA_SECRET",
        "SECRET_TURNSTILE_SECRET",
        "SECRET_HA_PASSWORD",
    ]
    ...
    MAP_SECRET_VARS = [
        ...
        "mcp_servers_auth_secrets",
        "mcp_servers_oauth_secrets",
    ...
    def test_map_secrets_declared_as_map_string(self, db):
        ...
        for var_name in self.MAP_SECRET_VARS:
            ...
            assert "map(string)" in section, f"{var_name} should be map(string), got: {section.strip()}"
    ...
    def test_string_secrets_declared_as_string(self, db):
        ...
        for var_name in self.STRING_SECRET_VARS:
            assert f'variable "{var_name}"' in var_tf, f"{var_name} not declared"
            section = var_tf.split(f'variable "{var_name}"')[1].split("}")[0]
            assert "type        = string" in section, f"{var_name} should be string, got: {section.strip()}"
            assert "map(string)" not in section, f"{var_name} should NOT be map(string), got: {section.strip()}"
    ...
    def test_all_secret_vars_marked_sensitive(self, db):
        ...
        for var_name in all_vars:
            section = var_tf.split(f'variable "{var_name}"')[1].split("}")[0]
            assert "sensitive   = true" in section, f"{var_name} missing sensitive = true"
    ...
    def test_map_secret_placeholders_in_secrets_tfvars(self, db):
        ...
        for var_name in self.MAP_SECRET_VARS:
            assert f"{var_name} = {{" in secrets, f"{var_name} should have map placeholder in dev.secrets.tfvars"
        ...
        assert '"tools" = "change-me"' in secrets, "mcp_servers_auth_secrets should have 'tools' key pre-populated"
    ...
    def test_no_secret_map_vars_in_dev_tfvars(self, db):
        ...
        tv = _tfvars(zf)
        for var_name in self.MAP_SECRET_VARS:
            assert f"{var_name} = {{" not in tv, f"{var_name} should not be in dev.tfvars (belongs in secrets.tfvars)"
    ...
    def test_corex_password_in_secrets_tfvars_not_dev_tfvars(self, db):
        ...
        assert "\ncorex_password =" not in tv, "corex_password should not be assigned in dev.tfvars"
        assert 'corex_password = "change-me"' in secrets, "corex_password should be assigned in dev.secrets.tfvars"
    ...
    def test_tfvars_example_has_map_placeholders(self, db):
        ...
        for var_name in self.MAP_SECRET_VARS:
            assert f"{var_name} = {{" in example, f"{var_name} missing map placeholder in tfvars.example"
        ...
        assert '"tools" = "change-me"' in example, "tfvars.example should have 'tools' key in mcp_servers secrets"
    ...
    def test_tfvars_example_has_string_placeholders(self, db):
        ...
        example = _read(zf, "terraform.tfvars.example")
        for var_name in self.STRING_SECRET_VARS:
            assert f'{var_name} = "change-me"' in example, f"{var_name} missing string placeholder in tfvars.example"
    ...
    def test_tfvars_example_absent_when_master_on(self, db):
        ...
        for var_name in all_vars:
            assert (
                f"{var_name} =" not in example or "change-me" not in example.split(f"{var_name}")[1].split("\n")[0]
    ...
    def test_no_duplicate_secret_vars_in_module(self, db):
        ...
        for mod_name in ["management", "mcp-gateway", "ssl"]:
            ...
            for var_name in self.MAP_SECRET_VARS + self.STRING_SECRET_VARS:
                ...
                assert count <= 1, f"{var_name} declared {count} times in {mod_name}/variables.tf"
    ...
    def test_captcha_secrets_have_placeholders_when_off(self, db):
        ...
        assert "captcha_secrets" in secrets, "captcha_secrets map should be in dev.secrets.tfvars"
        ...
        assert '"cap_secret"' not in tv, "cap_secret should not be in dev.tfvars (it's in secrets.tfvars)"
        assert '"recaptcha_secret"' not in tv, "recaptcha_secret should not be in dev.tfvars"
        assert '"turnstile_secret"' not in tv, "turnstile_secret should not be in dev.tfvars"
    ...
    def test_every_sensitive_setting_has_placeholder(self, db):
        ...
        for key in SENSITIVE_SETTING_KEYS:
            ...
            assert (
                f'{key} = "change-me"' in combined
                or f'"{key}" = "change-me"' in combined
...
class TestRuntimeStateExclusion:
    @pytest.mark.parametrize(
        ...
        [
            "last_applied_at",
            "geoip_download_last_run_at",
            "rule_set_download_poll_last_run_at",
            "auto_renew_last_run_at",
            "security_list_feeds_poll_last_run_at",
            "captcha_cookie_secret",
            "pp_hasher_bypass_token",
            "ja4_auto_disabled_rule_ids",
            "ja4_risk_auto_disabled_rule_ids",
            "risk_auto_disabled_rule_ids",
            "api_armor_profiler_interval",
            "api_armor_schema_learn_interval",
            "crs_active_version",
        ],
    )
    def test_runtime_state_excluded(self, db, key):
        ...
        db.commit()
        zf = _export(
            db, include_secrets=True, include_certs=True, include_users_identities=True, include_system_secrets=True
        )
        tv = _tfvars(zf)
...
class TestSingletonSettings:
    ...
    def test_singleton_resources_in_management_module(self, db):
        ...
        zf = _export(db, include_system_secrets=True)
        mgmt_tf = _module_tf(zf, "management")
        for res in ["corex_global_options", "corex_captcha_settings", "corex_ha_config"]:
...
class TestExternalFiles:
    def test_cert_pem_files_generated(self, db):
        ...
        assert not any("le_cert" in n and ".pem" in n for n in names), "Let's Encrypt cert should NOT have PEM files"
    ...
    def test_file_expressions_use_try(self, db):
        ...
        for line in file_lines:
            if line.startswith("#"):
                continue
            assert "try(" in line, f"file() expression not wrapped in try(): {line}"
    ...
    def test_error_page_file_name_matches_key(self, db):
        ...
        assert not any("error__403" in n for n in names), "File should not have double underscore"
        # Verify the root locals.tf expression uses ${k} which matches the key
        locals_tf = _read(zf, "locals.tf")
        assert "error_${k}.html" in locals_tf, "Root locals.tf should use error_${k}.html (matching the raw code key)"
...
class TestRootWiring:
    ...
    def test_no_unexpected_args_empty_db(self, db):
        ...
        assert not mismatches, f"Root passes arguments that modules don't declare: {mismatches}"
    ...
    def test_no_circular_module_dependency(self, db):
        ...
        assert "module.routing" in traffic_block, "Traffic module should depend on routing (one-way)"
        ...
        for mod in ["routing", "traffic", "waf", "cache", "management", "mcp-gateway"]:
            ...
            for v in mod_vars:
                ...
                if v in (
                    ...
                    "mcp_skill_bodies",
                    "mcp_skill_frontmatters",
                ):
                    assert f"{v} = " in main_tf, f"Module '{mod}' var '{v}' not wired in root main.tf"
                else:
                    assert f"{v} = var.{v}" in main_tf, f"Module '{mod}' var '{v}' not wired in root main.tf"
...
class TestHCLValidity:
    def test_resource_blocks_have_for_each(self, db):
        ...
        for name in zf.namelist():
            ...
            singleton_types = {
                "corex_captcha_settings",
                "corex_ha_config",
                "corex_api_armor_settings",
                "corex_global_options",
                "corex_maxmind_license_key",
                "corex_page_protect_settings",
                "corex_mcp_alert_config",
    ...
    def test_no_duplicate_resource_types_in_module(self, db):
        ...
        for name in zf.namelist():
            ...
            assert len(types) == len(set(types)), f"{name}: duplicate resource types: {types}"
    ...
    def test_module_tf_balanced_braces(self, db):
        ...
        for name in zf.namelist():
            if not name.startswith("modules/") or not name.endswith("/main.tf"):
                continue
            content = _read(zf, name)
            assert content.count("{") == content.count("}"), f"Unbalanced braces in {name}"
...
class TestFieldPreservation:
    def test_backend_all_fields_preserved(self, db):
        ...
        for skipped_field in ["timeout_queue", "timeout_check", "timeout_tunnel", "http_reuse", "fullconn"]:
            assert skipped_field not in fields, (
                f"Backend field '{skipped_field}' should be skipped (not in provider schema)"
            )
    ...
    def test_waf_rule_all_fields_preserved(self, db):
        ...
        for skipped in [
            ...
            assert f"{skipped} = " not in waf_tf, (
                f"WAF rule field '{skipped}' should be skipped (not in provider schema)"
            )
    ...
    def test_name_included_in_tfvars(self, db):
        ...
        assert m is not None
        assert '"name" = "web-server"' in m.group(1), "name field should preserve original hyphenated name"
    ...
    def test_optional_fields_use_try_all_modules(self, db):
        ...
        for mod_name in [
            ...
            assert not bare_refs, f"Found bare each.value references (no try()) in {mod_name} module: {bare_refs}"
...
class TestTypedVariables:
    def test_collection_vars_use_typed_objects(self, db):
        ...
        for mod_name in ["routing", "security-lists", "cache"]:
            try:
                ...
                continue
            assert "map(object({" in mod_vars, f"{mod_name} variables.tf should use map(object(...)) types"
        # Check root variables.tf
        root_vars = zf.read("variables.tf").decode()
        assert "map(object({" in root_vars, "Root variables.tf should use map(object(...)) types"
    ...
    def test_typed_vars_use_optional(self, db):
        ...
        assert "optional(string)" in sec_vars, "Typed fields should use optional()"
    ...
    def test_typed_vars_no_any_for_collections(self, db):
        ...
        assert not collection_any_vars, f"Collection variables should not use type=any: {collection_any_vars}"
...
class TestStateSeparation:
    def test_backend_tf_has_no_hardcoded_key(self, db):
        ...
        assert 'key = "corex/terraform.tfstate"' not in backend_tf, "backend.tf should not hardcode a single state key"
        # Should mention per-environment backend config
        assert "backend-config=environments" in backend_tf, "backend.tf should reference per-environment backend config"
...
class TestNestedCacheRules:
    def test_cache_rules_nested_under_configs(self, db):
        ...
        db.add(
            CacheRule(
                cache_config_id=cc.id, priority=0, match_type="path", pattern="/static/*", action="cache", tier="memory"
            )
        )
        db.add(
            CacheRule(
                cache_config_id=cc.id, priority=1, match_type="path", pattern="/api/*", action="bypass", tier="memory"
            )
        )
        db.commit()
        zf = _export(db)
        ...
        assert "cache_rules = {" not in tv, "cache_rules should not be a separate collection in tfvars"
        # cache_configs should have nested rules
        m = re.search(r"cache_configs = \{(.*?)\n\}", tv, re.DOTALL)
        assert m is not None, "cache_configs not found in tfvars"
        assert '"rules"' in m.group(1), "cache_configs should have nested rules"
        assert '"pattern" = "/static/*"' in m.group(1), "First rule pattern should be in nested rules"
        assert '"pattern" = "/api/*"' in m.group(1), "Second rule pattern should be in nested rules"
    ...
    def test_cache_module_flattens_nested_rules(self, db):
        ...
        assert 'resource "corex_cache_rule" "this"' in cache_tf, "Cache module should have corex_cache_rule resource"
        # Should use merge() to flatten nested rules
        assert "merge([" in cache_tf, "Cache rules should use merge() to flatten nested rules"
...
class TestMcpSelfRegisteredExclusion:
    def test_platform_team_included(self, db):
        ...
        assert "platform" in entries, "Platform team should be included (user-created resources may live in it)"
    ...
    def test_corex_manager_server_excluded(self, db):
        ...
        db.add(McpServer(name="corex-manager", team_id=team.id, namespace="corex-manager", transport_type="stdio"))
        db.commit()
        zf = _export(db)
        tv = _tfvars(zf)
...
class TestSingletonSchema:
    ...
    def test_global_options_uses_list_object(self, db):
        ...
        db.add(
            Setting(
                key="haproxy_global_options", value='[{"target":"global","directive":"log","value":"","enabled":true}]'
            )
        )
        db.commit()
        zf = _export(db)
        ...
        assert "jsondecode(" not in block, "Should not use jsondecode() — the tfvars already has a list"
...
class TestFeedManagedLists:
    def test_feed_managed_list_marked_in_tfvars(self, db):
        ...
        assert "feed_managed" in tor_fields, "Feed-managed list should have feed_managed field in tfvars"
    ...
    def test_feed_managed_list_no_snapshot_file(self, db):
        ...
        assert tor_file not in zf.namelist(), "Feed-managed list should not ship a snapshot file"
        # The owned list SHOULD have a JSON file
        owned_file = "environments/dev/files/security-lists/network_owned_list.json"
        assert owned_file in zf.namelist(), "Owned list should ship a snapshot file"
    ...
    def test_feed_managed_list_has_ignore_changes(self, db):
        ...
        assert 'corex_network_list" "feed_managed"' in sec_tf, "Should have a feed_managed resource block"
    ...
    def test_owned_list_not_in_feed_managed_block(self, db):
        ...
        assert 'resource "corex_network_list" "this"' in sec_tf, "Owned list should be in .this resource block"
    ...
    def test_root_entries_skip_feed_managed(self, db):
        ...
        assert "feed_managed" in locals_tf, "Root locals.tf should reference feed_managed in entries expression"
...
class TestImportBlocks:
    ...
    def test_import_feed_managed_targets_feed_managed_block(self, db):
        ...
        assert 'id = "tor-exit-nodes"' in imports, "Feed-managed import ID should be the original API name"
    ...
    def test_import_server_uses_numeric_id(self, db):
        ...
        assert f'id = "{srv.id}"' in imports, "Server should be imported by numeric row ID"
    ...
    def test_import_backend_rule_uses_numeric_id(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert f'id = "{br.id}"' in imports, "BackendRule should be imported by numeric row ID"
    ...
    def test_import_cache_config_uses_backend_name(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert 'id = "web"' in imports, "CacheConfig should be imported by backend name, not numeric backend_id"
    ...
    def test_import_user_uses_username(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert 'id = "admin"' in imports, "User should be imported by username"
    ...
    def test_import_setting_uses_key(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert 'id = "site_name"' in imports, "Setting should be imported by its key"
    ...
    def test_import_mcp_team_member_uses_composite_names(self, db):
        ...
        team = Team(name="Eng", slug="eng")
        user = User(username="admin", email="admin@ex.com", role="admin", hashed_password="$2b$12$abc", is_admin=True)
        ...
        imports = _read(zf, "imports.tf")
        composite = "Eng:admin"
        assert f'id = "{composite}"' in imports, "McpTeamMember should be imported by composite team_name:user_username"
    ...
    def test_import_waf_exception_uses_numeric_id(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert f'id = "{we.id}"' in imports, "WafException should be imported by numeric row ID"

    def test_import_vector_sink_uses_name(self, db):
        """VectorSink import ID should be the sink name."""
        db.add(VectorSink(name="s3", type="aws_s3", source="corex", options={"bucket": "logs", "region": "us-east-1"}))
        db.commit()
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert 'id = "s3"' in imports, "VectorSink should be imported by name"
    ...
    def test_import_mcp_server_replica_uses_numeric_id(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert f'id = "{rep.id}"' in imports, "McpServerReplica should be imported by numeric row ID"
    ...
    def test_import_readme_uses_module_prefix(self, db):
        ...
        assert "module.routing.corex_backend.this" in readme, "README should show module-prefixed import addresses"
        assert "module.ssl.corex_certificate.this" in readme, "README should show module-prefixed import addresses"
    ...
    def test_empty_db_no_unexpected_args(self, db):
        ...
        for mod_name in [
            ...
            if mod_block_match:
                ...
                unexpected = passed - declared
                assert not unexpected, f"Module {mod_name} gets unexpected args: {unexpected}"
...
class TestEnvironmentVariable:
    ...
    def test_environment_in_dev_tfvars(self, db):
        ...
        zf = _export(db)
        tv = _tfvars(zf)
        assert 'environment = "dev"' in tv, "dev.tfvars should set environment = 'dev'"
    ...
    def test_environment_comment_explains_copying(self, db):
        ...
        zf = _export(db)
        tv = _tfvars(zf)
        assert "prod" in tv.lower(), "dev.tfvars should mention prod in the environment comment"
...
class TestHaproxyGlobalOptionsType:
    ...
    def test_root_variable_typed_as_list_object(self, db):
        ...
        block = m.group(1)
        assert "list(object({" in block, "haproxy_global_options should be typed as list(object({...})) at root"
        ...
        assert "default     = []" in block, "haproxy_global_options should default to [] at root"
...
class TestCaptchaSecretsSplit:
    ...
    def test_captcha_secrets_in_secrets_tfvars(self, db):
        ...
        assert '"captcha_provider" = "recaptcha"' in tv, "Non-secret captcha key should be in dev.tfvars"
        # Secret keys should be in secrets.tfvars, not dev.tfvars
        assert "captcha_secrets" in secrets, "captcha_secrets map should be in dev.secrets.tfvars"
        assert '"cap_secret"' in secrets, "cap_secret should be in dev.secrets.tfvars"
        assert '"recaptcha_secret"' in secrets, "recaptcha_secret should be in dev.secrets.tfvars"
        # Secret keys should NOT be in dev.tfvars
        assert '"cap_secret"' not in tv, "cap_secret should not be in dev.tfvars"
        assert '"recaptcha_secret"' not in tv, "recaptcha_secret should not be in dev.tfvars"
    ...
    def test_captcha_module_references_captcha_secrets(self, db):
        ...
        zf = _export(db)
        mgmt_tf = _module_tf(zf, "management")
        assert "var.captcha_secrets" in mgmt_tf, "Module should reference var.captcha_secrets for secret keys"
        assert "var.captcha_settings" in mgmt_tf, "Module should reference var.captcha_settings for non-secret keys"
    ...
    def test_captcha_secrets_inlined_with_include_secrets(self, db):
        ...
        assert "captcha_secrets" not in tv, "captcha_secrets should not exist when include_secrets=True"
...
class TestUsersUsernameField:
    ...
    def test_user_tfvars_includes_username(self, db):
        ...
        zf = _export(db)
        tv = _tfvars(zf)
        assert '"username" = "admin"' in tv, "User tfvars should include username field explicitly"
...
class TestProductionSafety:
    ...
    def test_listeners_have_prevent_destroy(self, db):
        ...
        zf = _export(db)
        routing_tf = _module_tf(zf, "routing")
        assert "prevent_destroy = true" in routing_tf, "Listeners should have prevent_destroy"
    ...
    def test_imports_tf_explains_adoption_not_config_generation(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "adopt" in imports.lower(), "imports.tf should explain that blocks adopt existing objects"
    ...
    def test_readme_import_section_mentions_imports_tf(self, db):
        ...
        zf = _export(db)
        readme = _read(zf, "README.md")
        assert "imports.tf" in readme, "README should mention the generated imports.tf file"
...
class TestGitignore:
    def test_lockfile_not_ignored(self, db):
        ...
        for line in gi.strip().split("\n"):
            ...
            if line.startswith("#"):
                continue
            assert ".terraform.lock.hcl" not in line, f".terraform.lock.hcl should not be ignored: {line}"
...
class TestDynamicFeedTargetListId:
    ...
    def test_target_list_id_is_string_in_tfvars(self, db):
        ...
        assert '"tor_exit_nodes"' in tv, "target_list_id should be a string key in tfvars"
...
class TestFeedManagedLookup:
    ...
    def test_feed_uses_local_not_direct_reference(self, db):
        ...
        assert "local.network_list_ids" in sec_tf, "Polymorphic FK should use local.network_list_ids"
        ...
        if coalesce_m:
            assert "local." in coalesce_m.group(1), "coalesce() should reference locals, not direct resource refs"
    ...
    def test_locals_no_feed_managed_for_owned_types(self, db):
        ...
        if m:
            ...
            if network_m:
                assert "merge(" not in network_m.group(1), "Should not merge when no feed-managed lists"
    ...
    def test_merged_ids_exported_as_output(self, db):
        ...
        assert "network_list_ids" in outputs_tf, "Merged ID map should be exported as output"
...
class TestMcpSkillVersions:
    ...
    def test_variable_always_declared(self, db):
        ...
        assert 'variable "mcp_skill_versions"' in mod_vars, "mcp_skill_versions should always be declared in the module"
    ...
    def test_self_registered_skill_versions_filtered_from_tfvars(self, db):
        ...
        assert "version_corex_manager_v1" not in tv, "Self-registered skill version should be filtered from tfvars"
        # The user skill version SHOULD be in tfvars
        assert "version_my_tool_v1" in tv, "User skill version should be in tfvars"
    ...
    def test_self_registered_skill_version_no_import_block(self, db):
        ...
        assert "version_unknown" not in imports, "Self-registered skill version should not get an import block"
        assert "version_corex_manager" not in imports, "Self-registered skill version should not get an import block"
        # Should have an import for the user skill version
        assert "version_my_tool_v1" in imports, "User skill version should get an import block"
...
class TestWafExceptionFiltering:
    ...
    def test_waf_exception_without_rule_id_included(self, db):
        ...
        assert '"with_rule"' in tv, "WAF exception with waf_rule_id should be in tfvars"
        assert '"orphan"' in tv, "WAF exception without waf_rule_id should be in tfvars (nullable)"
        # Both should have import blocks
        assert "orphan" in imports, "WAF exception without waf_rule_id should get an import block"
...
class TestNoDuplicateModuleArgs:
    ...
    def test_no_duplicate_captcha_secrets_on_management(self, db):
        ...
        assert count == 1, f"captcha_secrets should appear once in management block, found {count}"
...
class TestCacheRuleImports:
    ...
    def test_cache_rule_imports_use_composite_keys(self, db):
        ...
        assert 'id = "1"' in imports or 'id = "2"' in imports, "Cache rule import ID should be the numeric row ID"

    def test_cache_rule_imports_absent_when_no_configs(self, db):
        """No cache rule imports when there are no cache configs."""
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "corex_cache_rule" not in imports, "No cache rule imports when there are no cache configs"
...
class TestMcpTeamIdFiltering:
    ...
    def test_platform_server_included_in_tfvars(self, db):
        ...
        assert "opensearch" in servers, "Server on platform team should be included (user-created)"
        assert "tools" in servers, "Server on eng team should be present"
    ...
    def test_platform_identity_included_in_tfvars(self, db):
        ...
        assert "platform_bot" in identities, "Identity on platform team should be included"
        assert "eng_bot" in identities, "Identity on eng team should be present"
    ...
    def test_platform_policy_included_in_tfvars(self, db):
        ...
        assert "platform_pol" in policies, "Policy on platform team should be included"
        assert "eng_pol" in policies, "Policy on eng team should be present"
    ...
    def test_platform_server_included_in_module(self, db):
        ...
        assert 'resource "corex_mcp_server" "this"' in mod_tf, "MCP server resource block should exist"
    ...
    def test_platform_server_replica_included(self, db):
        ...
        assert any("plat" in k for k in replicas), "Replica of platform-team server should be included"
        assert any("eng" in k for k in replicas), "Replica of eng-team server should be present"
    ...
    def test_eng_server_has_team_id(self, db):
        ...
        assert "team_id" in fields, "Server should have team_id field in tfvars"
        # Verify the team_id value is the sanitized team name
        import re as _re

        m = _re.search(r'"opensearch" = \{([^}]*)\}', tv)
        assert '"team_id" = "corex_engineering"' in m.group(1), "team_id should be the sanitized team name"
...
class TestStructuralRecommendations:
    ...
    def test_api_armor_settings_in_api_armor_module(self, db):
        ...
        mgmt = _module_tf(zf, "management")
        aa = _module_tf(zf, "api-armor")
        assert "corex_api_armor_settings" not in mgmt, "api_armor_settings should NOT be in management module"
        assert "corex_api_armor_settings" in aa, "api_armor_settings should be in api-armor module"

    def test_pattern_list_resource_always_emitted(self, db):
        """corex_pattern_list resource should be emitted even when empty."""
        zf = _export(db)
        sec_tf = _module_tf(zf, "security-lists")
        assert 'resource "corex_pattern_list" "this"' in sec_tf, "Pattern list resource block should always be emitted"
    ...
    def test_skill_version_always_has_body_frontmatter(self, db):
        ...
        block = m.group(1)
        assert "var.mcp_skill_bodies" in block, "skill_version should reference var.mcp_skill_bodies"
        assert "var.mcp_skill_frontmatters" in block, "skill_version should reference var.mcp_skill_frontmatters"
    ...
    def test_locals_in_separate_file(self, db):
        ...
        assert "locals {" not in sec_tf, "Locals should not be in main.tf — use locals.tf"
        # Locals SHOULD be in locals.tf
        assert "locals {" in sec_locals, "Locals should be in locals.tf"
    ...
    def test_root_outputs_include_waf_rule_ids(self, db):
        ...
        zf = _export(db)
        outputs = _read(zf, "outputs.tf")
        assert "waf_waf_rule_ids" in outputs, "Root outputs should include waf_rule_ids"
    ...
    def test_root_outputs_include_mcp_team_ids(self, db):
        ...
        zf = _export(db)
        outputs = _read(zf, "outputs.tf")
        assert "mcp_gateway_mcp_team_ids" in outputs, "Root outputs should include mcp_team_ids"

    def test_api_key_list_id_uses_string_key(self, db):
        """Auth policy api_key_list_id should be a string key, not numeric ID."""
        from app.models.api_armor import ApiKeyList, AuthPolicy

        akl = ApiKeyList(name="test-keys")
        db.add(akl)
        db.flush()
        db.add(AuthPolicy(name="test-policy", auth_type="api_key", api_key_list_id=akl.id, enabled=True))
        db.commit()
        zf = _export(db)
        tv = _tfvars(zf)
        ...
        aa_tf = _module_tf(zf, "api-armor")
        assert "corex_api_armor_api_key_list.this" in aa_tf, "Module should resolve api_key_list_id via resource lookup"
    ...
    def test_singleton_imports_generated(self, db):
        ...
        assert "module.management.corex_ha_config.this" in imports, "ha_config singleton should have an import block"
        ...
        assert "cannot be imported" not in imports, "The 'cannot be imported' comment should be removed"
...
class TestGeneratorHygiene:
        ...
        for name in zf.namelist():
            if name.endswith("locals.tf") and "modules/" in name:
                ...
                if mod_name != "security-lists":
                    content = _read(zf, name)
                    assert content.strip() != "", f"{mod_name}/locals.tf should not be emitted empty"
    ...
    def test_no_stub_readme(self, db):
        ...
        for name in zf.namelist():
            if name.endswith("README.md") and "modules/" in name:
                ...
                assert "See `main.tf`" not in content, f"{name} is a stub README and should not be emitted"
    ...
    def test_root_file_locals_block(self, db):
        ...
        assert "locals {" in root_locals, "Root locals.tf should have a locals block for file content"
        root_main = _read(zf, "main.tf")
    ...
    def test_no_inline_file_in_module_args(self, db):
        ...
        if idx >= 0:
            ...
            assert "file(" not in mod_block, "Module arguments should use local.xxx, not inline file()"
...
class TestFeedManagedAllTypes:
    ...
    def test_geo_list_always_has_feed_managed_block(self, db):
        ...
        zf = _export(db)
        sec_tf = _module_tf(zf, "security-lists")
        assert 'resource "corex_geo_list" "this"' in sec_tf, "Geo list should have .this resource block"
    ...
    def test_ja4_list_always_has_feed_managed_block(self, db):
        ...
        zf = _export(db)
        sec_tf = _module_tf(zf, "security-lists")
        assert 'resource "corex_ja4_list" "this"' in sec_tf, "JA4 list should have .this resource block"
    ...
    def test_pattern_list_always_has_feed_managed_block(self, db):
        ...
        zf = _export(db)
        sec_tf = _module_tf(zf, "security-lists")
        assert 'resource "corex_pattern_list" "this"' in sec_tf, "Pattern list should have .this resource block"
    ...
    def test_all_list_types_use_merge_in_locals(self, db):
        ...
        from app.models.models import GeoList, NetworkList
        ...
        for type_key in ["network", "asn", "geo", "ja4", "pattern"]:
            ...
            assert f"corex_{type_key}_list.feed_managed" in sec_locals, f"{type_key} merge should include .feed_managed"
    ...
    def test_feed_coalesce_includes_all_list_types(self, db):
        ...
        db.add(nl)
        db.flush()
        db.add(DynamicFeed(name="test-feed", list_type="network", target_list_id=nl.id, url="https://example.com"))
        db.commit()
        zf = _export(db)
        sec_tf = _module_tf(zf, "security-lists")
        # All 5 list types should be in the coalesce
        assert "local.network_list_ids[each.value.target_list_id]" in sec_tf, "coalesce should include network_list_ids"
        assert "local.asn_list_ids[each.value.target_list_id]" in sec_tf, "coalesce should include asn_list_ids"
        assert "local.geo_list_ids[each.value.target_list_id]" in sec_tf, "coalesce should include geo_list_ids"
        assert "local.ja4_list_ids[each.value.target_list_id]" in sec_tf, "coalesce should include ja4_list_ids"
        assert "local.pattern_list_ids[each.value.target_list_id]" in sec_tf, "coalesce should include pattern_list_ids"
        # The tobool() trick should be gone
        assert "tobool(" not in sec_tf, "tobool() trick should be removed — use plain coalesce()"
...
class TestApiKeyListType:
    ...
    def test_api_key_lists_has_name_and_entries(self, db):
        ...
        assert "name" in block, "api_key_lists type should include name field"
        assert "entries" in block, "api_key_lists type should include entries field"
        assert "value" in block, "entries type should include value field"
...
class TestProviderSchemaCompatibility2:
    ...
    def test_cache_config_provider_field_names(self, db):
        ...
        cc = CacheConfig(
            ...
            haproxy_total_max_size=1024,
            haproxy_max_age=3600,
    ...
    def test_listener_no_options_field(self, db):
        ...
        db.add(be)
        db.flush()
        ln = Listener(name="https", bind_address="0.0.0.0", bind_port=443, mode="http", default_backend_id=be.id)
        db.add(ln)
        db.commit()
        zf = _export(db)
        tv = _tfvars(zf)
    ...
    def test_mcp_team_member_no_name(self, db):
        ...
        db.add(team)
        db.flush()
        user = User(username="admin", role="admin", email="a@b.com", hashed_password="dummy")
        db.add(user)
        db.flush()
        ut = UserTeam(team_id=team.id, user_id=user.id)
        db.add(ut)
    ...
    def test_mcp_skill_version_no_name_no_version(self, db):
        ...
        db.add(skill)
        db.flush()
        sv = McpSkillVersion(skill_id=skill.id, version=1, created_by="admin", body="content")
        db.add(sv)
        db.commit()
        zf = _export(db)
        mcp_tf = _module_tf(zf, "mcp-gateway")
        # version is computed-only — should not be set in config
        assert "version = try(each.value.version" not in mcp_tf, "version is computed-only, should not be set"
    ...
    def test_mcp_policy_no_priority(self, db):
        ...
        db.add(team)
        db.flush()
        pol = McpPolicy(name="allow-all", team_id=team.id, priority=5, expression="true", action="allow")
        db.add(pol)
        db.commit()
        zf = _export(db)
        mcp_tf = _module_tf(zf, "mcp-gateway")
        ...
        assert "priority = " not in block, "priority is computed-only, should not be set in mcp_policy"
    ...
    def test_dns_credentials_secret_var_is_map_of_maps(self, db):
        ...
        cert = Certificate(name="wildcard", provider="letsencrypt", dns_credentials='{"api_key": "secret"}')
        db.add(cert)
        db.commit()
        ...
        assert "map(map(string))" in block, "certificates_dns_credentials should be map(map(string))"
    ...
    def test_dns_credentials_secret_placeholder_is_empty_map(self, db):
        ...
        assert '"wildcard" = {}' in secrets, "dns_credentials placeholder should be empty map, not 'change-me'"
    ...
    def test_mcp_skill_frontmatter_is_string_not_object(self, db):
        ...
        db.add(skill)
        db.flush()
        sv = McpSkillVersion(skill_id=skill.id, version=1, body="content", frontmatter='{"name": "test"}')
        db.add(sv)
        db.commit()
        zf = _export(db)
        locals_tf = _read(zf, "locals.tf")
...
class TestNewResourcesAndDeadSecrets:
    ...
    def test_page_protect_settings_exported(self, db):
        ...
        assert 'resource "corex_page_protect_settings" "this"' in pp_tf, "page_protect_settings should be exported"
    ...
    def test_page_protect_script_exported(self, db):
        ...
        db.add(
            PageProtectScript(
                url="https://example.com/script.js", resource_type="script", ignored=False, source="manual"
            )
        )
        db.commit()
        zf = _export(db)
        pp_tf = _module_tf(zf, "page-protect")
        assert 'resource "corex_page_protect_script" "this"' in pp_tf, "page_protect_script should be exported"
    ...
    def test_ssl_labs_settings_exported(self, db):
        ...
        zf = _export(db)
        ssl_tf = _module_tf(zf, "ssl")
        assert 'resource "corex_ssl_labs_settings" "this"' in ssl_tf, "ssl_labs_settings should be exported"
    ...
    def test_mcp_alert_config_exported(self, db):
        ...
        zf = _export(db)
        mcp_tf = _module_tf(zf, "mcp-gateway")
        assert 'resource "corex_mcp_alert_config" "this"' in mcp_tf, "mcp_alert_config should be exported"
    ...
    def test_no_dead_user_secret_vars(self, db):
        ...
        db.add(User(username="admin", email="a@b.com", role="admin", hashed_password="dummy"))
        db.commit()
        zf = _export(db)  # all flags off
        var_tf = _read(zf, "variables.tf")
        secrets = _read(zf, "environments/dev.secrets.tfvars")
        ...
        assert 'variable "users_totp_secrets"' not in var_tf, "users_totp_secrets should not be declared (dead secret)"
        assert "users_hashed_passwords" not in secrets, "users_hashed_passwords should not be in secrets tfvars"
        assert "users_totp_secrets" not in secrets, "users_totp_secrets should not be in secrets tfvars"
    ...
    def test_no_dead_mcp_identity_pat_hash_secret_var(self, db):
        ...
        db.add(team)
        db.flush()
        db.add(McpIdentity(name="bot", team_id=team.id, kind="pat", pat_hash="SECRET_PAT"))
        db.commit()
        zf = _export(db)  # all flags off
        var_tf = _read(zf, "variables.tf")
        secrets = _read(zf, "environments/dev.secrets.tfvars")
        ...
        assert "mcp_identities_pat_hashes" not in secrets, "mcp_identities_pat_hashes should not be in secrets tfvars"
    ...
    def test_user_resource_has_no_hashed_password_or_totp(self, db):
        ...
        db.add(User(username="admin", email="a@b.com", role="admin", hashed_password="dummy", totp_secret="secret"))
        db.commit()
        zf = _export(db)
        mgmt_tf = _module_tf(zf, "management")
        ...
        assert "hashed_password" not in block, "user resource should not have hashed_password (not in provider)"
        assert "totp_secret" not in block, "user resource should not have totp_secret (not in provider)"
    ...
    def test_page_protect_settings_in_imports(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "corex_page_protect_settings" in imports, "page_protect_settings should have import block"
    ...
    def test_mcp_alert_config_in_imports(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "corex_mcp_alert_config" in imports, "mcp_alert_config should have import block"
    ...
    def test_ssl_labs_settings_in_imports(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "corex_ssl_labs_settings" in imports, "ssl_labs_settings should have import block"
    ...
    def test_page_protect_script_in_imports(self, db):
        ...
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "corex_page_protect_script" in imports, "page_protect_script should have import block"
        assert "https://example.com/script.js" in imports, "page_protect_script import should use URL as ID"

    def test_no_page_protect_settings_skip_comment(self, db):
        """imports.tf should not have the old 'not yet exported' comment."""
        zf = _export(db)
        imports = _read(zf, "imports.tf")
        assert "not yet exported" not in imports, "imports.tf should not say page_protect_settings is not yet exported"
...
class TestSchemaMismatchFixes:
    ...
    def test_cache_config_no_name_attribute(self, db):
        ...
        assert "name = " not in block, "cache_config should not have name attribute (not in provider schema)"
    ...
    def test_ssl_labs_settings_typed_as_map_object(self, db):
        ...
        assert "map(object({" in rv, "ssl_labs_settings root variable should be map(object({...}))"
    ...
    def test_ssl_labs_tfvars_no_cert_id(self, db):
        ...
        tv = _tfvars(zf)
        assert "ssl_labs_settings" in tv
        assert '"cert_id"' not in tv, "cert_id should not be in tfvars (wired to local cert resource)"
    ...
    def test_mcp_server_env_vars_secret_is_map(self, db):
        ...
        db.add(
            McpServer(
                name="tools", team_id=team.id, namespace="eng", transport_type="stdio", env_vars_json='{"FOO": "bar"}'
            )
        )
        db.commit()
        zf = _export(db, include_secrets=False, include_system_secrets=False)
        ...
        assert "map(map(string))" in var_tf, "mcp_servers_env_vars should be map(map(string))"
        # Placeholder should be an empty map, not "change-me"
        secrets = _read(zf, "environments/dev.secrets.tfvars")
        assert '"tools" = {}' in secrets, "env_vars placeholder should be empty map, not string"
    ...
    def test_mcp_server_args_is_list_in_tfvars(self, db):
        ...
        db.add(
            McpServer(
                name="tools", team_id=team.id, namespace="eng", transport_type="stdio", args_json='["--port", "8080"]'
            )
        )
        db.commit()
        zf = _export(db, include_secrets=True)
        tv = _tfvars(zf)
        assert '"args" = ["--port", "8080"]' in tv, "args should be a list in tfvars, not a JSON string"
    ...
    def test_mcp_server_env_vars_is_map_in_tfvars(self, db):
        ...
        zf = _export(db, include_secrets=True)
        tv = _tfvars(zf)
        assert '"env_vars" = {"FOO" = "bar"}' in tv, "env_vars should be a map in tfvars, not a JSON string"
    ...
    def test_mcp_dlp_rule_no_priority(self, db):
        ...
        db.add(
            McpDlpRule(
                team_id=team.id, name="rule1", direction="request", detector="regex", find_regex="test", action="block"
            )
        )
        db.commit()
        zf = _export(db)
        ...
        block = mcp_tf[idx : idx + 500]
        assert "priority = " not in block, "mcp_dlp_rule should not have priority (computed-only)"
    ...
    def test_mcp_guardrail_no_priority(self, db):
        ...
        db.add(team)
        db.flush()
        db.add(McpGuardrail(team_id=team.id, name="g1", direction="both", find_regex="test", action="block"))
        db.commit()
        zf = _export(db)
        mcp_tf = _module_tf(zf, "mcp-gateway")
        idx = mcp_tf.find("corex_mcp_guardrail")
        block = mcp_tf[idx : idx + 500]
        assert "priority = " not in block, "mcp_guardrail should not have priority (computed-only)"
    ...
    def test_mcp_skill_no_enable_when_ast(self, db):
        ...
        block = mcp_tf[idx : idx + 500]
        assert "enable_when_ast = " not in block, "mcp_skill should not have enable_when_ast (computed-only)"
    ...
    def test_cipher_suite_tls_options_is_list(self, db):
        ...
        db.add(CipherSuite(name="modern", baseline="modern", ciphers="ECDHE", tls_options="no-sslv3 no-tlsv10"))
        db.commit()
        zf = _export(db)
        tv = _tfvars(zf)
        # Should be a list, not a string
        assert '"tls_options" = ["no-sslv3", "no-tlsv10"]' in tv, "tls_options should be a list in tfvars"
    ...
    def test_cipher_suite_tls_options_variable_type(self, db):
        ...
        assert "tls_options = optional(list(string))" in ssl_vars, "tls_options should be list(string) in variable type"
