...
from app.models.models import NetworkList, NetworkListEntry, PatternList, SecurityRule
...
class TestSecurityRulesAPI:
    ...
    def test_reorder(self, client, db):
        r1 = client.post(
            "/api/v1/security-rules", json={"name": "R1", "expression": 'http.host = "a"', "action": "block"}
        ).json()
        r2 = client.post(
            "/api/v1/security-rules", json={"name": "R2", "expression": 'http.host = "b"', "action": "block"}
        ).json()
        r3 = client.post(
            "/api/v1/security-rules", json={"name": "R3", "expression": 'http.host = "c"', "action": "block"}
        ).json()
        # Reorder: R3, R1, R2
...
class TestJa4Toggle:
    def test_toggle_off_disables_ja4_rules(self, client, db):
        ...
        client.post(
            ...
            json={
                "name": "ja4-rule",
                "expression": 'http.request.ja4 = "t13d1516h2_abc"',
                "action": "block",
            },
        )
        client.post(
            ...
            json={
                "name": "path-rule",
                "expression": 'http.request.uri.path = "/foo"',
                "action": "block",
            },
        )
