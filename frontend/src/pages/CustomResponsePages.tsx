...
export default function CustomResponsePages() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-border bg-card p-6 shadow-sm space-y-3 max-w-3xl">
        ...
        <p className="text-sm text-muted-foreground font-semibold">{t('pages:customResponsePages.templateVars.common')}</p>
        <ul className="text-sm text-muted-foreground list-disc ps-5 space-y-1">
          <li><code className="text-secondary-foreground">{'{{ request_id }}'}</code> — {t('pages:customResponsePages.templateVars.vars.requestId')}</li>
          <li><code className="text-secondary-foreground">{'{{ client_ip }}'}</code> — {t('pages:customResponsePages.templateVars.vars.clientIp')}</li>
          <li><code className="text-secondary-foreground">{'{{ client_port }}'}</code> — {t('pages:customResponsePages.templateVars.vars.clientPort')}</li>
          <li><code className="text-secondary-foreground">{'{{ method }}'}</code> — {t('pages:customResponsePages.templateVars.vars.method')}</li>
          <li><code className="text-secondary-foreground">{'{{ uri }}'}</code> — {t('pages:customResponsePages.templateVars.vars.uri')}</li>
          <li><code className="text-secondary-foreground">{'{{ path }}'}</code> — {t('pages:customResponsePages.templateVars.vars.path')}</li>
          <li><code className="text-secondary-foreground">{'{{ query }}'}</code> — {t('pages:customResponsePages.templateVars.vars.query')}</li>
          <li><code className="text-secondary-foreground">{'{{ host }}'}</code> — {t('pages:customResponsePages.templateVars.vars.host')}</li>
          <li><code className="text-secondary-foreground">{'{{ user_agent }}'}</code> — {t('pages:customResponsePages.templateVars.vars.userAgent')}</li>
          <li><code className="text-secondary-foreground">{'{{ referer }}'}</code> — {t('pages:customResponsePages.templateVars.vars.referer')}</li>
          <li><code className="text-secondary-foreground">{'{{ timeout }}'}</code> — {t('pages:customResponsePages.templateVars.vars.timeout')}</li>
          <li><code className="text-secondary-foreground">{'{{ timestamp }}'}</code> — {t('pages:customResponsePages.templateVars.vars.timestamp')}</li>
          <li><code className="text-secondary-foreground">{'{{ frontend_name }}'}</code> / <code className="text-secondary-foreground">{'{{ backend_name }}'}</code> — {t('pages:customResponsePages.templateVars.vars.proxyNames')}</li>
          <li><code className="text-secondary-foreground">{'{{ rate_limit_window }}'}</code> — {t('pages:customResponsePages.templateVars.vars.rateLimitWindow')}</li>
          <li><code className="text-secondary-foreground">{'{{ rate_limit_duration }}'}</code> — {t('pages:customResponsePages.templateVars.vars.rateLimitDuration')}</li>
        </ul>
        <p className="text-sm text-muted-foreground">
      ...
      <div className="rounded-lg border border-border bg-card p-6 shadow-sm space-y-3">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground">{t('pages:customResponsePages.noPages')}</p>
        ) : (
          <table className="w-full text-sm text-start">
            <thead className="text-muted-foreground border-b border-border"><tr><th>{t('pages:customResponsePages.tableHeaders.code')}</th><th>{t('pages:customResponsePages.tableHeaders.listener')}</th><th>{t('pages:customResponsePages.tableHeaders.contentType')}</th><th>{t('pages:customResponsePages.tableHeaders.length')}</th><th></th></tr></thead>
            <tbody>
              {items.map((ep) => (
